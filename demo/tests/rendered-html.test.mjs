...
test("server-renders the evaluation dashboard", async () => {
  ...
  assert.match(html, /Evaluation coverage/);
  assert.match(html, /Failure agreement/);
  assert.match(html, /Glossary/);
  assert.match(html, /What do the two frameworks say/);
  assert.match(html, /There is no absolute winner/);
...
test("source includes all interactive evidence flows", async () => {
  ...
  assert.match(page, /setLang/);
  assert.match(page, /document\.documentElement\.lang = lang/);
  assert.match(page, /const viQuestions: Record<string,string>/);
  assert.equal((page.match(/[EMHA]\d\d:"/g) ?? []).length, 20);
  assert.match(page, /const glossary = \[/);
  assert.equal((page.match(/\{term:/g) ?? []).length, 8);
  assert.match(page, /difficultyProfile/);
  assert.match(page, /agreement\.map/);
  assert.match(page, /RAGAS produced 72\/80 valid cells/);
  assert.match(page, /Không có framework thắng tuyệt đối/);
  assert.match(page, /Provider limitation/);
  assert.match(css, /conic-gradient/);
  assert.match(css, /\.difficulty-chart/);
  assert.match(css, /\.glossary-grid/);
  assert.match(css, /\.comparison-copy/);
